...
const MOCK_ALLY_WALLET = "GCEZWKCA5VLDNRLN3RPRJMRZOX3Z6G5CHCGBDQCQZVQQ6BRVV12BKHA";
...
test.describe("Evidence Lifecycle - Ally Workflow", () => {
  test("ally submits evidence with client-side hashing and enters submitted state", async ({
    ...
    await page.getByPlaceholder("https://").fill("https://example.com/statement.pdf");
  ...
  test("ally sees operator rejection reason and can resubmit", async ({ page }) => {
    ...
    await page.getByPlaceholder("https://").fill("https://example.com/corrected.pdf");
  ...
  test("paused payout contract blocks ally submissions", async ({ page }) => {
    ...
    await expect(
      page.getByText("The payout contract is paused. New submissions cannot be processed."),
    ).toBeVisible();
...
test.describe("Evidence Lifecycle - Operator Review Queue", () => {
  test("operator starts review and approves a submitted cycle", async ({ page }) => {
    const scenario = new PilotRpcScenario().cycle("2026-03").submitted(
  ...
  test("operator rejects a cycle with a required reason", async ({ page }) => {
    ...
    const reasonInput = page.getByPlaceholder("Required on rejection or dispute...");
    await reasonInput.fill("Discrepancy between bank records and reported revenue.");
  ...
  test("paused contract disables operator review actions", async ({ page }) => {
    ...
    await expect(
      page.getByText("The payout contract is paused. Review actions are unavailable."),
    ).toBeVisible();
...
test.describe("Evidence Lifecycle - Investor Route", () => {
  test("investor timeline reflects on-time, late, disputed, and unreceived cycles", async ({
    ...
    await mockConnectedWallet(page, "GDB6EXAMPLEWALLETADDRESS1234567890123456789012345678901234");
    await mockPilotRpc(page, scenario);
