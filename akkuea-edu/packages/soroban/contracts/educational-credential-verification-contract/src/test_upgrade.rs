...
use soroban_sdk::{
    ...
    vec, Address, BytesN, Env, IntoVal, Map, String, Vec,
};
...
fn test_full_upgrade_cycle() {
    ...
    let educators_valid =
        client.validate_migration_integrity(&admin, &String::from_str(&env, "educators"));
    let credentials_valid =
        client.validate_migration_integrity(&admin, &String::from_str(&env, "credentials"));
