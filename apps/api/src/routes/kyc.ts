...
export const kycRoutes = new Elysia({ prefix: '/kyc' })
  .get('/status/:userId', async ({ params: { userId }, headers, set }) => {
    try {
      const requester = (headers as Record<string, string | undefined>)['x-user-address'];
      if (!requester || requester !== userId) {
        set.status = 403;
        return { success: false, error: 'FORBIDDEN', message: 'You can only view your own KYC status' };
      }
  ...
  .post('/verify/:documentId', async ({ params: { documentId }, body, headers, set }) => {
    try {
      if (!isInternalOperationsAuthorized(headers as Record<string, string | undefined>)) {
        set.status = 403;
        return { success: false, error: 'FORBIDDEN', message: 'Internal API key required' };
      }
