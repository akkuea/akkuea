  ...
  ({ label, error, hint, leftIcon, rightIcon, className, id, ...props }, ref) => {
    ...
    return (
      <div className="w-full">
        {label && (
          <label htmlFor={inputId} className="block text-xs font-medium text-neutral-400 mb-2">
            {label}
          </label>
        )}
        <div className="relative">
          {leftIcon && (
            <div className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-500">{leftIcon}</div>
          )}
          <input
            ref={ref}
            id={inputId}
        ...
        {hint && !error && <p className="mt-1.5 text-[10px] text-neutral-600 font-mono">{hint}</p>}
