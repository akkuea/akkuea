...
describe.skipIf(skipIfNoDatabase)(
  'E2E: tokenization → share purchase → collateral lending',
  () => {
    // -----------------------------------------------------------------------
    ...
    describe('Step 1: Tokenize property', () => {
      it('tokenizes the property and persists contract address + sorobanPropertyId', async () => {
        ...
        const result = await PropertyController.tokenizeProperty(
          propertyId,
          {},
          OWNER_ADDRESS,
        );
        ...
        const [dbPosition] = await db
          ...
          .where(
            and(
              ...
              eq(borrowPositions.borrowerId, buyerKycOkId),
            ),
          );
