...
## Two-party signing payload transport (C8-001, distribution/evidence/exit co-signing)

**Adopted: no relay.** The partially-authorized transaction that needs both the operator's and the ally's signature (`execute_distribution`, and the same pattern for `record_evidence` and `exit`) travels as a plain JSON string the operator hands to the ally directly, by whatever channel they already use to communicate (a link, a QR code, a pasted message). No server stores the unsigned invocation or the collected authorization entries at any point.

**What actually makes this work:** `AssembledTransaction` in `@stellar/stellar-sdk/contract` (the same class every generated pilot contract client already returns) has this exact two-party pattern as a first-class, documented feature: `toJson()`/`fromJson()` serialize and reconstruct the transaction under assembly, `needsNonInvokerSigningBy()` reports which addresses still owe a signature, and `signAuthEntries()` lets the non-invoking party (the ally) sign their own authorization entry without touching the invoker's (the operator's) key. This module (`apps/webapp/src/services/pilot/cosign.ts`) does not invent a transport or envelope format; it calls this API directly and layers pilot-specific checks (contract paused/exited, cycle-status staleness, the EURC price floor) on top.

**Why this preserves, not just satisfies, the "no key material, no signed payload through the API" property `writes.ts` already documents:** a relay would need to store the unsigned invocation and the ally's signed authorization entry somewhere the operator's browser can later retrieve them. Even with no private key ever touching that storage, that is still new state describing a real fund movement, sitting on a server, needing its own access control (the issue's own text: "restricted to the two configured signer addresses"), expiry, and cleanup. The no-relay path has none of that: the payload only ever exists in the two browsers involved and whatever channel carries it between them, which is exactly as private as the channel the operator and ally already use to communicate today.

**Why not weigh this against a relay's UX benefit and choose it anyway:** the pilot has exactly two signers, coordinating a payout once per cycle (roughly monthly, see `product-brief.md`). A relay's main advantage, an in-app "pending co-signature" inbox instead of a manually-shared link, is a real but small convenience at that frequency, and it comes with a genuinely new piece of production infrastructure (storage, auth, expiry, cleanup, an availability dependency for a step that "is the moment real money leaves the contract," per the issue's own quality bar). For a pilot at this scale, a manually-shared payload is not a shortcut standing in for the real solution; it is the right-sized solution.

**Expiry:** the shared payload embeds no expiry of its own (a `toJson()` blob is just a snapshot). What actually gates validity on-chain is the ledger sequence at which the ally's authorization entry expires, set when the ally calls `signAuthEntries({ expiration })`. The SDK's own default (about 8.3 minutes) assumes both parties are at their keyboards together, which this flow explicitly does not assume (the issue: "The operator and ally will usually sign on different machines at different times"). `cosign.ts` instead defaults to 24 hours, configurable via `NEXT_PUBLIC_PILOT_COSIGN_EXPIRY_HOURS`, and the confirmation screen shows the resulting expiration so both parties can see how long the request stays valid before it needs to be re-prepared.
