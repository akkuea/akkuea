...
export const lendingRoutes = new Elysia({ prefix: '/lending' })
  ...
  .post('/pools', ({ body }) => LendingController.createPool(body as Partial<import('@real-estate-defi/shared').LendingPool>))
  .get('/pools/:id', ({ params: { id } }) => LendingController.getPool(id))
  .post('/pools/:id/deposit', ({ params: { id }, body }) => LendingController.deposit(id, body as { user: string; amount: number }))
  .post('/pools/:id/borrow', ({ params: { id }, body }) => LendingController.borrow(id, body as { borrower: string; collateralPropertyId: string; collateralShares: number; borrowAmount: number }))
