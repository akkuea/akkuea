...
export const propertyRoutes = new Elysia({ prefix: '/properties' })
  ...
  .post('/', ({ body }) => PropertyController.createProperty(body as Partial<import('@real-estate-defi/shared').PropertyInfo>))
  .post('/:id/tokenize', ({ params: { id }, body }) =>
    PropertyController.tokenizeProperty(id, body as unknown),
  )
  .post('/:id/buy-shares', ({ params: { id }, body }) => PropertyController.buyShares(id, body as { buyer: string; shares: number }))
