...
export const lendingRoutes = new Elysia({ prefix: '/lending' })
  ...
  .get('/pools/:id/user/:address/deposits', ({ params: { id, address } }) => LendingController.getUserDeposits(id, address))
  .get('/pools/:id/user/:address/borrows', ({ params: { id, address } }) => LendingController.getUserBorrows(id, address));