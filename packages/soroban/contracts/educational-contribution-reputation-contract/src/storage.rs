...
pub fn get_rate_limit_data(env: &Env, key: String) -> Option<RateLimitData> {
    env.storage().instance().get(&DataKey::RateLimit(key))
}
...
pub fn get_circuit_breaker_state(env: &Env, key: String) -> Option<CircuitBreakerState> {
    env.storage().instance().get(&DataKey::CircuitBreaker(key))
}
...
pub fn store_external_credential(env: &Env, credential: &ExternalCredential) {
    env.storage().instance().set(
        &DataKey::ExternalCredential(credential.id.clone()),
        credential,
    );
}
...
pub fn store_professional_certification(env: &Env, certification: &ProfessionalCertification) {
    env.storage().instance().set(
        &DataKey::ProfessionalCert(certification.id.clone()),
        certification,
    );
}

/// Get professional certification
pub fn get_professional_certification(
    env: &Env,
    certification_id: String,
) -> Option<ProfessionalCertification> {
    env.storage()
        .instance()
...
pub fn store_user_professional_certifications(
    env: &Env,
    user_id: u64,
...
pub fn get_user_import_export_operations(env: &Env, user_id: u64) -> Vec<u64> {
    ...
    let search_limit = if max_operation_id > 100 {
        max_operation_id - 100
    } else {
        1
    };
...
pub fn cleanup_expired_credentials_batch(env: &Env, max_users: u32) {
    ...
    let user_ids = get_all_user_ids(env);
    let process_count = if user_ids.len() > max_users {
        max_users
    } else {
        user_ids.len()
    };
...
pub fn store_user_verification(env: &Env, verification: &UserVerification) {
    env.storage().instance().set(
        &DataKey::UserVerification(verification.user_id),
        verification,
    );
}
...
pub fn store_verification_delegation(env: &Env, delegation: &VerificationDelegation) {
    env.storage().instance().set(
        &DataKey::VerificationDelegation(delegation.delegate.clone(), delegation.user_id),
        delegation,
    );
}

/// Get verification delegation for specific delegate and user
pub fn get_verification_delegation(
    env: &Env,
    delegate: &Address,
    user_id: u64,
) -> Option<VerificationDelegation> {
    env.storage()
        .instance()
